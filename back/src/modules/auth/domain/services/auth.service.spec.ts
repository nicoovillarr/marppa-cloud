...
import {
  TokenGenerator,
  TOKEN_GENERATOR_SYMBOL,
} from './token-generator.service';
import {
  TokenStorageService,
  TOKEN_STORAGE_SERVICE_SYMBOL,
...
describe('AuthService', () => {
  ...
  describe('generateAndSaveUserTokens', () => {
    it('should generate access and refresh tokens and save them', async () => {
      ...
      expect(tokenGenerator.generateJwt).toHaveBeenCalledWith(
        mockUser,
        'access',
      );
      expect(tokenGenerator.generateJwt).toHaveBeenCalledWith(
  ...
  describe('createSessionForUser', () => {
    it('should create a session for a user', async () => {
      ...
      const sessionArg = (repository.createSession as jest.Mock).mock
        .calls[0][0];
  ...
  describe('findSessionByRefreshToken', () => {
    it('should return a session by refresh token', async () => {
      ...
      const result =
        await service.findSessionByRefreshToken('refresh-token-123');

      expect(repository.findSessionByRefreshToken).toHaveBeenCalledWith(
        'refresh-token-123',
      );
      expect(result).toEqual(mockSession);
    });
  ...
  describe('getTokenInformation', () => {
    it('should return token information for valid token', async () => {
      ...
      expect(tokenGenerator.validateJwt).toHaveBeenCalledWith(
        'refresh-token-123',
      );
    ...
    it('should return null for invalid token', async () => {
      mockTokenGenerator.validateJwt.mockRejectedValue(
