...
export class WorkerFamilyApiService {
  ...
  async findById(id: number): Promise<WorkerFamilyResponseModel> {
    ...
    return plainToInstance(WorkerFamilyResponseModel, entity, { excludeExtraneousValues: true });
  }

  async create(data: CreateWorkerFamilyDto): Promise<WorkerFamilyResponseModel> {
    const entity = await this.service.create(data);
    return plainToInstance(WorkerFamilyResponseModel, entity, { excludeExtraneousValues: true });
  }

  async update(id: number, data: UpdateWorkerFamilyDto): Promise<WorkerFamilyResponseModel> {
