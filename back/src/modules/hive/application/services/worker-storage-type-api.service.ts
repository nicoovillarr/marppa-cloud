...
import { WorkerStorageTypeModel } from "../models/worker-storage-type.model";
...
export class WorkerStorageTypeApiService {
  ...
  async findById(id: number): Promise<WorkerStorageTypeModel> {
    const entity = await this.service.findById(id);
    return plainToInstance(WorkerStorageTypeModel, entity, { excludeExtraneousValues: true });
  }

  async create(data: CreateWorkerStorageTypeDto): Promise<WorkerStorageTypeModel> {
    const entity = await this.service.createWorkerStorageType(data);
    return plainToInstance(WorkerStorageTypeModel, entity, { excludeExtraneousValues: true });
  }

  async update(id: number, data: UpdateWorkerStorageTypeDto): Promise<WorkerStorageTypeModel> {
