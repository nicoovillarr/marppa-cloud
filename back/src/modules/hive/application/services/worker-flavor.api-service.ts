...
export class WorkerFlavorApiService {
  ...
  async findById(id: number): Promise<WorkerFlavorResponseModel> {
    ...
    return plainToInstance(WorkerFlavorResponseModel, entity, { excludeExtraneousValues: true });
  }

  async create(data: CreateWorkerFlavorDto): Promise<WorkerFlavorResponseModel> {
    const entity = await this.service.createWorkerFlavor(data);
    return plainToInstance(WorkerFlavorResponseModel, entity, { excludeExtraneousValues: true });
  }

  async update(id: number, data: UpdateWorkerFlavorDto): Promise<WorkerFlavorResponseModel> {
