...
export class NodeController {
  ...
  @Get()
  public async findByZoneId(@Param('zoneId') zoneId: string): Promise<NodeResponseModel[]> {
    return this.apiService.findByZoneId(zoneId);
  }

  @Post()
  public async create(@Param('zoneId') zoneId: string, @Body() data: CreateNodeDto): Promise<NodeResponseModel> {
    return this.apiService.create(zoneId, data);
  }

  @Get(':nodeId')
  public async findById(@Param('zoneId') zoneId: string, @Param('nodeId') nodeId: string): Promise<NodeResponseModel> {
    return this.apiService.findById(zoneId, nodeId);
  }

  @Delete(':nodeId')
  public async delete(@Param('zoneId') zoneId: string, @Param('nodeId') nodeId: string): Promise<void> {
