...
import { ZoneRepository, ZONE_REPOSITORY_SYMBOL } from '../repositories/zone.repository';
...
describe('ZoneService', () => {
    ...
    describe('findWithNodesById', () => {
        it('should return a zone with nodes by id', async () => {
            mockZoneRepository.findWithNodesById.mockResolvedValue(mockZoneWithNodesModel);
        ...
        it('should throw NotFoundError if zone not found', async () => {
            ...
            await expect(service.findWithNodesById('z-999999')).rejects.toThrow(NotFoundError);
        });
    });
    ...
    describe('create', () => {
        ...
        it('should throw UnauthorizedError if no user in session', async () => {
            ...
            expect(() => service.create(createDto, cidr, gateway)).toThrow(UnauthorizedError);
        });
    });
    ...
    describe('update', () => {
        ...
        it('should throw UnauthorizedError if no user in session', async () => {
            ...
            await expect(service.update('z-000001', updateDto)).rejects.toThrow(UnauthorizedError);
        });

        it('should throw NotFoundError if zone not found', async () => {
             mockZoneRepository.findById.mockResolvedValue(null);

             await expect(service.update('z-999999', updateDto)).rejects.toThrow(NotFoundError);
        });
    });
