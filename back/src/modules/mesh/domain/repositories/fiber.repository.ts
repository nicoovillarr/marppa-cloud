...
export abstract class FiberRepository {
  abstract findById(zoneId: string, nodeId: string, fiberId: number): Promise<FiberEntity | null>;
  ...
  abstract delete(zoneId: string, nodeId: string, fiberId: number): Promise<void>
}