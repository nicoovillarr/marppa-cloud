...
export class TransponderUpdateProcessor implements IEventProcessor {
  readonly eventType = EventType.TRANSPONDER_UPDATE;
  ...
  async handle(event: EventPayload): Promise<void> {
    ...
    try {
      ...
      const originalStatus = transponder.status as ResourceStatus;
      await updateTransponderStatus(ResourceStatus.PROVISIONING);

      await this.orbitService.generateNginxConfig(transponder.portal);

      await updateTransponderStatus(originalStatus);

      const eventCreated = await this.repository.createEvent(EventType.TRANSPONDER_UPDATED, event.createdBy, event.companyId);
