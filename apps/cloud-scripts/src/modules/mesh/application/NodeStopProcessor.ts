...
const STATES = getEventStates(EventType.NODE_STOP);

/**
 * Turn a node off: drop its DHCP reservation and detach the worker NIC from the
 * zone bridge, leaving the node INACTIVE. Unlike NODE_UNASSIGN_WORKER the worker
 * stays attached to the node, so NODE_START can bring it back with the same IP.
 */
@EventProcessor(EventType.NODE_STOP)
export class NodeStopProcessor implements IEventProcessor {
  ...
  public async handle(event: EventPayload): Promise<void> {
    ...
    try {
      ...
      const createdEventId = await this.repository.createEvent(EventType.NODE_STOPPED, event.createdBy, event.companyId);
