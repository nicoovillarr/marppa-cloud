...
@EventProcessor(EventType.ATOM_DELETE)
export class AtomDeleteProcessor implements IEventProcessor {
  constructor(
  ...
  public async handle(event: EventPayload): Promise<void> {
    let atom: AtomWithNode | null = null;
    ...
    try {
      ...
      if (await this.nucleusService.isAtomRunning(atom.id)) {
        throw new AbortError(
          `Atom ${atom.id} is running`,
          EventType.ATOM_DELETE_FAILED,
        );
      }
