...
const Overview = () => {
  const { workers, zones, bits } = useAppStore(
    useShallow((state) => ({
  ...
  const createdBitsLastMonth = bits
    ?.filter(
      (bit) =>
        new Date(bit.createdAt).getMonth() === new Date().getMonth() - 1
    )
    .reduce((acc) => acc + 1, 0);
  ...
  const zoneNodesCount =
    zones
      ...
      .flatMap((zone: ZoneDTO) => zone.nodes || []).length || 0;
    ...
    <section className="w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card className="md:col-span-2 lg:col-span-1">
          ...
          <h4 className="font-bold text-3xl">{bits?.length || 0}</h4>
          <p className="text-sm text-gray-500">
            +{createdBitsLastMonth} from last month
          </p>
