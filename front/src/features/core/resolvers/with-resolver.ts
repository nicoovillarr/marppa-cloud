...
export function withResolver<R extends readonly Resolver<any>[]>(
  ...
  return function (
    ...
    return async function Wrapper({
      ...
      const result = (await Promise.all(apiCalls).then((combinedData) => {
        return combinedData.reduce((acc, act) => ({ ...acc, ...act }), {});
      })) as ResolverResult<R>;

      return React.createElement(PageComponent, { data: result });
    };
  };
}
